...
CMD su - postgres -c "pg_ctl start -D /var/lib/postgresql/data -l /var/lib/postgresql/data/logfile" && \
        sleep 5 && \
        psql -U postgres -c "DROP DATABASE IF EXISTS liveclinicdb ;" && \
        psql -U postgres -c "CREATE DATABASE liveclinicdb ;" && \
        psql -U postgres -d liveclinicdb  -f /app/database/liveclinicdb .sql && \
        npm start
